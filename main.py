...
6.  **Узел "messageNode" (Сообщение):**
    - `messageText` может содержать `{{{{переменная}}}}`.
    - Если есть кнопки, определи их в `data.buttons`: каждый объект кнопки должен иметь `id` (например, `"btn_confirm"`), `text` (например, `"Подтвердить"`) и `callback_data` (например, `"confirm_action"`).
    - **Критически Важно для Связей от Кнопок:** Каждая кнопка, которая должна инициировать какое-либо действие, должна иметь исходящую связь от своего уникального порта. Этот порт **всегда** формируется по шаблону `btn-out-ID_КНОПКИ`. Например, если у кнопки `id: "mood_great"`, то `sourceHandle` для ребра от этой кнопки должен быть `"btn-out-mood_great"`. **Не используй `callback_data` в качестве `sourceHandle`.**
    - Выход `message_output_main` используется, только если у "messageNode" нет кнопок, и поток должен просто продолжиться после отправки сообщения. 
    - Выход `message_output_main` используется, только если у "messageNode" нет кнопок и поток продолжается.
7.  **Сохранение callback_data от кнопки в переменную:**
    - Если после узла "messageNode" с кнопками нужно сохранить `callback_data` нажатой кнопки в переменную:
        a.  Создай один узел "setVariableNode".
        b.  В `data.variableName` этого узла укажи имя переменной (например, `userChoice`).
        c.  В `data.variableValue` этого узла укажи специальный плейсхолдер: `{{{{last_callback_data}}}}`. Платформа Nodera автоматически подставит сюда `callback_data` нажатой кнопки.
        d.  **Важно:** Каждая кнопка из узла "messageNode", чей `callback_data` нужно сохранить, должна иметь исходящую связь от своего порта (который имеет формат `btn-out-ID_КНОПКИ`) к этому единственному узлу "setVariableNode".
